...
motorML = Motor(Ports.PORT8, GearSetting.RATIO_6_1, False)  # mid-left
motorMR = Motor(Ports.PORT2,  GearSetting.RATIO_6_1, True)   # mid-right

rotationMotor = Motor(Ports.PORT11, GearSetting.RATIO_6_1, True) #claw rotation motor
clawPinchers = DigitalOut(brain.three_wire_port.a)
clawRotation = DigitalOut(brain.three_wire_port.b)
extensionL = Motor(Ports.PORT5, GearSetting.RATIO_6_1, True)   # left extension motor
extensionR = Motor(Ports.PORT6, GearSetting.RATIO_6_1, False) # right extension motor

# ---- Sensors ----
inertialSensor     = Inertial(Ports.PORT7)
trackingWheelVert  = Rotation(Ports.PORT4, True)  # vertical tracking wheel  (forward/back)
trackingWheelHoriz = Rotation(Ports.PORT5, True)   # horizontal tracking wheel (strafe)
...
def extension_move_to_position(target_position_turns, velocity):
    ...
    extensionR.spin_to_position(target_position_turns, TURNS, wait=False)

def claw_go_to_front():
    #make sure that robot is in starting position before this (inspection position)
    clawPinchers.set(False)
    clawRotation.set(True)
    rotationMotor.set_velocity(60,PERCENT)
    rotationMotor.set_stopping(HOLD)
    rotationMotor.spin_to_position(390, DEGREES, wait=True)
def claw_open():
    clawPinchers.set(True)
def claw_close():
    clawPinchers.set(False)
def motor_Stop():
...
def motor_Motion(motorFLSpeed, motorFRSpeed, motorBLSpeed, motorBRSpeed, motorMLSpeed, motorMRSpeed):
    ...
    # Note: the ML/MR arguments are intentionally swapped here to match wiring
...
def when_started1():
    ...
    then runs a 10-inch forward move as an example.
    Replace the autonomousPIDTracking call below with your actual auton sequence.
    """
    ...
    claw_close()
    claw_go_to_front()
    # rotationMotor.spin_to_position(0, DEGREES, wait=True)
    # rotationMotor.stop(HOLD)
    bias = -37
    ...
    for i in range(2):
        Time_wait   = 200   # observed max 178 iters; 2x buffer
        export_flag = 1
        f           = 5
        r           = bias
        v_min       = 60
        ...
        autonomousPID([f, math.radians(r)], v_min, v_max, Time_wait, export_flag,
                    Kp_linear, Kp_rotation)
    extension_move_to_position(1, 50)
    # drives to the alliance goal
    Time_wait   = 500   # observed max 178 iters; 2x buffer
    export_flag = 1
    f           = 14
    r           = bias
    v_min       = 60
    ...
    autonomousPID([f, math.radians(r)], v_min, v_max, Time_wait, export_flag,
                  Kp_linear, Kp_rotation)
    extension_move_to_position(3, 50)
    wait(0.75,SECONDS)
    ...
    extension_move_to_position(4, 50)
    time.sleep(0.75)
    claw_open()
    time.sleep(0.75)
    Time_wait   = 400   #observed max 178 iters; 2x buffer
